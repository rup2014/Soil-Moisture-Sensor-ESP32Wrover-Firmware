...
//U8G2_SH1106_128X64_NONAME_1_SW_I2C My_u8g_Panel(U8G2_R0, /* clock=*/ GPIO_NUM_13, /* data=*/ GPIO_NUM_15, /* reset=*/ GPIO_NUM_14);
U8G2_SSD1306_128X64_NONAME_1_SW_I2C My_u8g_Panel(U8G2_R0, /* clock=*/ GPIO_NUM_13, /* data=*/ GPIO_NUM_15, /* reset=*/ GPIO_NUM_34);
...
// pin used to power display
gpio_num_t displayPower = GPIO_NUM_14; 
RTC_DATA_ATTR bool displayPowerOn = true;
// Switch
...
void setup(void) {
  ...
  // Begin panel init (possibly causing the flicker)
  My_u8g_Panel.begin();
  u8g_prepare();
...
void onOledSwitchLongPressClick(){
...
void switchOledPower(){
  if(displayPowerOn == true){
    displayPowerOn = false;
  }else{
    displayPowerOn = true;
  }
  #ifdef DEBUG
    Serial.println(displayPowerOn);
  #endif
}
...
void draw(char* soilStatus) {
  //My_u8g_Panel.drawStr( 0, 10, "Hi lets see!");
...
void sleepNow(){
  ...
  #endif
  // if(displayPowerOn == true){
  //   // Used to set the gpio pin to high while in deepsleep
  //   gpio_set_direction(displayPower, GPIO_MODE_OUTPUT);
  //   // This will hold the output of gpio 9 to be high while in deepsleep
  //   gpio_hold_en(displayPower);
  // }else{
  //   gpio_hold_dis(displayPower);
  // }
